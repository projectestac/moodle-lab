...
function listaclase ($course,$activity,$group){
  ...
  $sql="SELECT id, student, alias FROM {$CFG->prefix}msociograma_sheet
...
//function to get the array with all of class students with alias
function listaclaseAlias ($course,$activity,$group){
  global $CFG, $USER,$COURSE,$DB;  
...
function tableCescChartjs ($nombre,$curs, $act,$grup, $arr,$coef,$title,$color,$alias){
	global $CFG, $USER,$COURSE, $DB;	
	...
	$log = $DB->get_records_sql($sql, array('%'));
	   ...
	   $arrDiagrama = $temp;
		...
		$cadenaArrayAlumnos ="'";
		if ($alias)
			$arrNom = listaclaseAlias ($curs,$act,$grup); 
		else
			$arrNom = listaclase ($curs,$act,$grup); 
...
function trioTableCescChartjs ($nombre,$curs, $act,$grup, $arr1,$coef1,$title1,$arr2,$coef2,$title2, $arr3,$coef3,$title3,$alias){
	global $CFG, $USER,$COURSE, $DB;	
...
//this function draw a tag cloud with the answers selected in the array $arr
function cloudTagsSheet ($curs, $act,$grup, $arr,$coef,$title,$colorbar,$alias){
	...
	//query for diagram. 
	$sql="SELECT a.stu_ans, s.student,s.alias, count(*) FROM {$CFG->prefix}msociograma_answers a join {$CFG->prefix}msociograma_sheet s
	WHERE a.stu_ans=s.id AND a.id in 
	(SELECT max(id) FROM {$CFG->prefix}msociograma_answers a 
	WHERE ($subsql) 
	and course = '$COURSE->id' AND activity = '$act' AND grup= '$grup' GROUP BY id_stu,question ORDER BY id) 
	...
	foreach($log as $entrada_log){
		...
		if (($entrada_log->$paraula)!=0)
			...
			if ($alias)
				$html=$html.'  <FONT FACE=arial COLOR="#000080" FONT SIZE='.$size .' STYLE="font-size: '.$pt.'pt">'.$entrada_log->alias.'&nbsp;</FONT>';
			else
