...
//msociograma is based in CESC sociogram, designed by Collell, J. and Escudé, C.
 ...
 //Shows the victim data in a chart
	...
	$html=$html. tableCescChartjs ('victimTot',$COURSE->id, $id, $currentgroup, array(9,10,11), 1, get_string('victimTot','msociograma').' ('.$_SESSION['group'].')','yellow' ,$alias);
	$html=$html. tableCescChartjs ('victimFis',$COURSE->id, $id, $currentgroup, array(9), 1, get_string('victimFis','msociograma').' ('.$_SESSION['group'].')','red',$alias );
	$html=$html. tableCescChartjs ('victimVerb',$COURSE->id, $id, $currentgroup, array(10), 1, get_string('victimVerb','msociograma').' ('.$_SESSION['group'].')','blue' ,$alias);
	$html=$html. tableCescChartjs ('victimRel',$COURSE->id, $id, $currentgroup, array(11), 1, get_string('victimRel','msociograma').' ('.$_SESSION['group'].')','green',$alias);
