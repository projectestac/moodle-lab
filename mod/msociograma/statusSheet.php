...
//msociograma is based in CESC sociogram, designed by Collell, J. and Escudé, C.
 ...
 //Shows the sociometric status data in a chart
	...
	$html=$html. tableCescChartjs ('selectPositiv',$COURSE->id, $id, $currentgroup, array(1), 1, get_string('selectPositiv','msociograma').' ('.$_SESSION['group'].')','red' ,$alias);
	$html=$html. cloudTagsSheet ($COURSE->id, $id,$currentgroup, array(1),1, get_string("selectPositiv", "msociograma").' ('.$_SESSION['group'].')','brownbar',$alias);
  
	$html=$html. tableCescChartjs ('selectNegativ',$COURSE->id, $id, $currentgroup, array(2), 1, get_string('selectNegativ','msociograma').' ('.$_SESSION['group'].')','blue' ,$alias);
	$html=$html.cloudTagsSheet ($COURSE->id, $id,$currentgroup, array(2),1,  get_string("selectNegativ", "msociograma").' ('.$_SESSION['group'].')','bluebar',$alias);
